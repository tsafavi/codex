...
def score_by_frequency(model, test_spo, direction="o"):
    ...
    scores = torch.zeros(
        [len(test_spo), model.dataset.num_entities()], dtype=torch.double
    )
    ...
    for i, (s, p, o) in enumerate(test_spo):
        ...
        scores[i, eids] = torch.tensor(eid_scores, dtype=torch.double)
...
def main():
    ...
    all_spo = torch.cat(
        (train_spo, valid_spo, test_spo), axis=0).type(torch.LongTensor)  # for filtering
